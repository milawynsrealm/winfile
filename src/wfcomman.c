...
BOOL FmifsLoaded()
{
    ...
    if (hfmifsDll < (HANDLE)32)
    {
        ...
        else
        {
            ...
            if (!lpfnFormat || !lpfnQuerySupportedMedia || !lpfnSetLabel || !lpfnDiskCopy)
            {
                MyMessageBox(hwndFrame, IDS_WINFILE, IDS_FMIFSLOADERR, MB_OK | MB_ICONEXCLAMATION | MB_SYSTEMMODAL);
...
{
	const WCHAR szBashFilename[] = TEXT("bash.exe");
	UINT len;
...
#define ConEmuParamFormat TEXT(" -Single -Dir \"%s\"")
            WCHAR szParams[MAXPATHLEN + COUNTOF(ConEmuParamFormat)];
...
#define PowerShellParamFormat TEXT(" -noexit -command \"cd \\\"%s\\\"\"")
        ...
        case IDM_DISKCOPY:
        {
            ...
            if (nFloppies == 1)
            {
                ...
                CancelInfo.Info.Copy.iSourceDrive = CancelInfo.Info.Copy.iDestDrive = i;
...
DealWithNetError:
        ...
        case IDM_CONNECTIONS:
        {
            ...
            ret = WNetConnectionDialog2(hwndFrame, RESOURCETYPE_DISK, wszWinfileHelp, IDH_CONNECTIONS);
        ...
        case IDM_CONNECT:
        {
            ...
            ret =  WNetConnectionDialog2(hwndFrame, RESOURCETYPE_DISK, wszWinfileHelp, IDH_CONNECT);
...
DealWithNetError_NotifyResume:
        ...
        case IDM_DISCONNECT:
        {
            ...
            ret = WNetDisconnectDialog2(hwndFrame, RESOURCETYPE_DISK, wszWinfileHelp, IDH_DISCONNECT);

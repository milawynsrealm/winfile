...
/*                                                                          */
/*--------------------------------------------------------------------------*/
INT_PTR CALLBACK ChooseDriveDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    WCHAR szDrive[5];
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
INT_PTR CALLBACK DiskLabelDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
...
FDC_RET FillDriveCapacity(HWND hDlg, INT nDrive, FMIFS_MEDIA_TYPE fmSelect, BOOL fDoPopup)
{
#if defined(DBCS)
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
INT_PTR CALLBACK FormatDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    WCHAR szBuf[128];
    INT  i, count;
    ...
    switch (wMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            if (CancelInfo.Info.Format.fFlags & FF_PRELOAD)
            {
                ...
                if (FillDriveCapacity(hDlg, CancelInfo.Info.Format.iFormatDrive, CancelInfo.Info.Format.fmMediaType, TRUE) == FDC_TRUE)
                {
                    /*
                ...
                case IDD_DRIVE:
                {
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        case CBN_SELCHANGE:
                        {
                            ...
                            if ((fdcr = FillDriveCapacity(hDlg, i, (DWORD) -1, TRUE)) != FDC_TRUE)
                            {
                                if (fdcr != FDC_FALSE_Q)
                                {
                                    // Popup message box
                                ...
                                FillDriveCapacity(hDlg, rgiDrive[nLastDriveInd], (DWORD) -1, FALSE);
                            }
                            break;
                        }
...
INT_PTR CALLBACK FormatSelectDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
INT_PTR CALLBACK AboutDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
...
BOOL Callback_Function(FMIFS_PACKET_TYPE PacketType, DWORD PacketLength, PVOID PacketData)
{
    ...
    switch (PacketType)
    {
        ...
        case FmIfsCantQuickFormat:
        {
            ...
            if (IDYES == SendMessage(hwndFrame, FS_CANCELMESSAGEBOX, IDS_FORMATERR, IDS_FORMATQUICKFAILURE))
            {
...
INT_PTR CALLBACK CancelDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_PAINT:
        {
            ...
            //
            if (CancelInfo.Info.Format.fQuick && CANCEL_FORMAT == CancelInfo.eCancelType)
            {
...
//
/////////////////////////////////////////////////////////////////////
INT_PTR CALLBACK ProgressDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
...
VOID CopyDiskEnd()
{
    ...
    LockFormatDisk(CancelInfo.Info.Copy.iSourceDrive, CancelInfo.Info.Copy.iDestDrive, 0, IDM_FORMAT, FALSE);
...
VOID LockFormatDisk(DRIVE drive1, DRIVE drive2, DWORD dwMessage, DWORD dwCommand, BOOL bLock)
{
    HMENU hMenu;
    ...
    if (dwCommand != IDM_FORMAT)
        EnableMenuItem(hMenu, dwCommand, bLock ? MF_BYCOMMAND | MF_GRAYED : MF_BYCOMMAND | MF_ENABLED);

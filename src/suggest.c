...
DWORD FormatError(BOOL bNullString, LPWSTR lpBuf, INT iSize, DWORD dwError)
{
    ...
    if ( !(dwError & DE_BEGIN) && !(pdwSuggest && pdwSuggest[1] & SUG_IGN_FORMATMESSAGE) )
    {
        ...
        do
        {
            dwNumChars = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK,
                                       NULL, dwError, wLangId, lpBuf, iSize*sizeof(lpBuf[0]), NULL );

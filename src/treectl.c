...
VOID GetTreePathIndirect(PDNODE pNode, register LPTSTR szDest)
{
...
VOID GetTreePath(PDNODE pNode, register LPWSTR szDest)
{
    szDest[0] = CHAR_NULL;
...
INT InsertDirectory(HWND hwndTreeCtl,
                    PDNODE pParentNode,
                    INT iParentNode,
                    LPTSTR szName,
                    PDNODE *ppNode,
             ...
             BOOL bPartialSort)
{
    ...
    if (!bFound && szAutoExpand && *szAutoExpand)
    {
        ...
        *szEndPath = CHAR_NULL;           // remove old stuff
        ...
        ReadDirLevel(hwndTreeCtl, pNode, szPath, uLevel+1, iNode, dwAttribs, bFullyExpand, szAutoExpand, bPartialSort);
    }
...
PDNODE FindParent( INT iLevelParent, INT iStartInd, HWND hwndLB)
{
    PDNODE pNode;
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
VOID FillTreeListbox(HWND hwndTC,
                     LPTSTR szDefaultDir,
                     BOOL bFullyExpand,
                     BOOL bDontSteal)
{
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
VOID FillOutTreeList(HWND hwndTC,
	                 LPWSTR szDir,
                     DWORD nIndex,
                     PDNODE pNode)
{
...
BOOL RectTreeItem(HWND hwndLB, INT iItem, BOOL bFocusOn)
{
    ...
    if (bFocusOn)
    {
        ...
        SetStatusText(SBT_NOBORDERS|255, SST_FORMAT|SST_RESOURCE,
                     (LPCTSTR)(DWORD)(fShowSourceBitmaps ? IDS_DRAG_COPYING : IDS_DRAG_MOVING), szPath);
        UpdateWindow(hwndStatus);
...
BOOL IsNetPath(PDNODE pNode)
{
    ...
    if (pNode->dwNetType == (DWORD)-1)
    {
        ...
        if (!aDriveInfo[drive].bShareChkFail && WNetGetDirectoryType( szPath,
            &dwType, !aDriveInfo[drive].bShareChkTried ) == WN_SUCCESS)
...
#undef szDir
        ...
        case WM_CHARTOITEM:
        {
            ...
            for (; j < cItems; j++)
            {
                ...
                if (CompareString( LOCALE_USER_DEFAULT, NORM_IGNORECASE, rgchMatch, cchMatch, pNode->szName, cchMatch) == 2)
                    break;
            }
            ...
            switch (wParam)
            {
                ...
                case FSC_MKDIRQUIET:
                {
                    ...
                    if (GetWindowLongPtr(hwndParent, GWL_VIEW) & VIEW_PLUSES)
                    {
                        ...
                        lstrcpy(szPath, (LPTSTR)lParam);
                        ScanDirLevel((PDNODE)pNodeT, szPath,
        ...
        case WM_VKEYTOITEM:
            ...
            switch (GET_WM_VKEYTOITEM_CODE(wParam, lParam))
            {
                ...
                case VK_RIGHT:
                {
                    ...
                    if ((SendMessage(hwndLB, LB_GETTEXT, i+1, (LPARAM)&pNodeNext) == LB_ERR) || (pNodeNext->pParent != pNode))
                        goto SameSelection;

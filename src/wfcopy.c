...
DWORD ConfirmDialog(
    HWND hDlg, DWORD dlg,
    LPWSTR pFileDest, PLFNDTA pDTADest,
    LPWSTR pFileSource, PLFNDTA pDTASource,
...
//
//--------------------------------------------------------------------------
LPWSTR StripColon(register LPWSTR pPath)
{
...
LPWSTR FindFileName(register LPWSTR pPath)
{
    register LPWSTR pT;
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
BOOL IsRootDirectory(register LPWSTR pPath)
{
...
//  0 = fail (general)
//  1 = succeed
BOOL IsTheDiskReallyThere(HWND hwnd, register LPWSTR pPath, DWORD dwFunc, BOOL bModal)
{
...
DWORD ConfirmDialog(
    HWND hDlg, DWORD dlg,
    LPWSTR pFileDest, PLFNDTA plfndtaDest,
    LPWSTR pFileSource, PLFNDTA plfndtaSrc,
...
LPWSTR pszDialogFrom;
LPWSTR pszDialogTo;

BOOL GetNameDlgProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    WCHAR szT[14];
    LPWSTR p;
    INT i, j, cMax, fDot;
...
DWORD GetNameDialog(DWORD dwOp, LPWSTR pFrom, LPWSTR pTo)
{
    DWORD dwRet = (DWORD)-1;
    DWORD dwSave;
    ...
    dwRet = (DWORD)DialogBox(hAppInstance, (LPWSTR) MAKEINTRESOURCE(LFNTOFATDLG),
...
DWORD WF_CreateDirectory(HWND hwndParent, LPWSTR szDest, LPWSTR szSrc)
{
    DWORD ret = 0;
...
ShowMessageBox:
            ...
            ret = CopyError(szSource, szDest, ret, pCopyInfo->dwFunc, oper, bErrorOnDest, bFatalError);
            ...
            switch (ret)
            {
                ...
                {
                    //
                    // Since we are canceling an op, we definitely know that
                    // an error occurred.
                    //
                    bErrorOccured = TRUE;
                    ret = 0;
    ...
    BOOL bCopy)
{
    ...
    if (bConfirmMouse)
    {
        LoadString(hAppInstance, bCopy ? IDS_COPYMOUSECONFIRM : IDS_MOVEMOUSECONFIRM, szTitle, COUNTOF(szTitle));
...
Error:
    ...
    dwStatus = DialogBoxParam(hAppInstance, (LPTSTR) MAKEINTRESOURCE(DMSTATUSDLG),
                              hwndFrame, ProgressDlgProc, (LPARAM)pCopyInfo);
...
;
============================================================================*/
DWORD CopyError(LPWSTR pszSource,
                LPWSTR pszDest,
                DWORD dwError,
                DWORD dwFunc,
                INT nOper,

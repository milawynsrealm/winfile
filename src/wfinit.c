    ...
    for (p = szBuf; *p && iNumExtensions < MAX_EXTENSIONS; p += lstrlen(p) + 1)
    {
        ...
        if (hMod)
        {
            ...
            if (fp)
            {
                ...
                {
                    if ((bUnicode ? lsW.dwSize : lsA.dwSize) != (bUnicode ? sizeof(FMS_LOADW) : sizeof(FMS_LOADA)))
                        goto LoadFail;
                    ...
                    if (hMenu)
                    {
                        ...
                        if (bUnicode)
                        {
                            InsertMenuW(hMenuFrame, iMenuBase + iMenuOffset,
                                MF_BYPOSITION | MF_POPUP, (UINT_PTR) hMenu, lsW.szMenuName);
                        }
                        else
                        {
                            InsertMenuA(hMenuFrame, iMenuBase + iMenuOffset,
                                MF_BYPOSITION | MF_POPUP, (UINT_PTR) hMenu, lsA.szMenuName);
                        }
                        iMenuOffset++;
                    }
...
/*                                                                          */
/*--------------------------------------------------------------------------*/
BOOL InitFileManager(HANDLE hInstance, LPWSTR lpCmdLine, INT nCmdShow)
{
    INT           i;
    HDC           hdcScreen;
...
// wndClass.lpszMenuName   = NULL;
    ...
    if (!hEventUpdate || !hEventUpdatePartial ||
        !hThreadUpdate || !hEventNetLoad || !hEventAcledit)
    {
        LoadFailMessage();
        bUpdateRun = FALSE;

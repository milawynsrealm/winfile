...
/*                                                                          */
/*--------------------------------------------------------------------------*/
INT_PTR CALLBACK SearchDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    LPWSTR p;
...
INT_PTR CALLBACK SuperDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    UINT len;
    LPWSTR pszFrom;
    //
...
SuperDlgExit:
                ...
                case IDOK:
                {
                    ...
                    //
                    if ( IDM_COPY == dwSuperDlgMode || IDM_MOVE == dwSuperDlgMode || IDM_RENAME == dwSuperDlgMode)
                        if (NoQuotes(szTo))
                            CheckEsc(szTo);
...
LPWSTR GetVersionDatum(LPTSTR pszName)
{
    DWORD cbValue = 0;
    LPWSTR lpValue;
...
VOID FillVersionList(HWND hDlg)
{
    LPWSTR lpszKey, lpszValue;
    ...
    if (!LoadString(hAppInstance, (cXlate == 1) ? IDS_VN_LANGUAGE : IDS_VN_LANGUAGES, szMessage, COUNTOF(szMessage)))
        goto Done;
...
/*--------------------------------------------------------------------------*/
INT_PTR CALLBACK AttribsDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    LPWSTR p, pSel;
    BOOL bRet;
    HCURSOR hCursor;

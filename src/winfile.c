...
VOID ResizeControls(VOID)
{
    ...
    MoveWindow(hwndMDIClient, rc.left-dyBorder, rc.top-dyBorder, rc.right, rc.bottom-rc.top+2*dyBorder-1, TRUE);
}
...
BOOL InitPopupMenus(UINT uMenus, HMENU hMenu, HWND hwndActive)
{
    ...
    if (uMenus & (1 << IDM_FILE))
    {
      ...
      /* can't print or edit a dir */
        ...
        if (pSel && !bCompressReEntry)
        {
            ...
            //
            if (GetVolumeInformation(szDrv, NULL, 0L, NULL, NULL, &dwFlags, NULL, 0L) && (dwFlags & FS_FILE_COMPRESSION))
            {
    ...
    if (uMenus & (1<<IDM_TREE))
    {
        ...
        if (hwndTree)
            CheckMenuItem(hMenu, IDM_ADDPLUSES, GetWindowLongPtr(hwndActive, GWL_VIEW) & VIEW_PLUSES ? 
    ...
    if (uMenus & (1<<IDM_VIEW))
    {
        ...
        CheckMenuItem(hMenu, IDM_VOTHER,  (dwView != VIEW_NAMEONLY && dwView != VIEW_EVERYTHING) ? 
            MF_CHECKED | MF_BYCOMMAND : MF_UNCHECKED | MF_BYCOMMAND);
...
DoDefault:
            ...
            if (wMsg == wHelpMessage)
            {
                ...
                else if (GET_WM_COMMAND_ID(wParam, lParam) == MSGF_DIALOGBOX)
                {
                    ...
                    if ((dwContext >= IDH_MBFIRST && dwContext <= IDH_MBLAST) || (dwContext == IDH_CTBAR))
                        WFHelp(hwnd);
                    else /* let dialog box deal with it */
...
ReturnFalse:
    ...
    if (hwndParent == hwndTree)
        if (SendMessage(hwndLB, LB_GETCURSEL, 0, 0L) > 0L && !GetWindowLongPtr(hwndTree, GWL_READLEVEL))
            return(TRUE);

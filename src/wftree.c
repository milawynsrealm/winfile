...
VOID SwitchDriveSelection(HWND hwndChild, BOOL bSelectToolbarDrive)
{
    DRIVE drive;
...
LRESULT CALLBACK TreeWndProc(register HWND hwnd, UINT uMsg, register WPARAM wParam, LPARAM lParam)
{
...
#undef lpmdics
        ...
        {
            if (HIWORD(lParam))
                EnableMenuItem((HMENU)wParam, SC_CLOSE, IsLastWindow() ? MF_BYCOMMAND | MF_DISABLED | MF_GRAYED : MF_ENABLED);

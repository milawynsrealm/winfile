...
LRESULT CALLBACK DirWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    INT iSel, i;
    HWND hwndLB;
    LPXDTA lpxdta;
    ...
    switch (uMsg)
    {
        ...
        case WM_CHARTOITEM:
        {
            ...
            for (; j < cItems; j++)
            {
                ...
                if (CompareString( LOCALE_USER_DEFAULT, NORM_IGNORECASE, rgchMatch, cchMatch, szItem, cchMatch) == 2)
                    break;
            }
...
CreateNewPath:
            ...
            //
            SetLBFont(hwnd, hwndLB, hFont, GetWindowLongPtr(hwndListParms, GWL_VIEW), lpStart);
...
//
/////////////////////////////////////////////////////////////////////
INT PutAttributes(register DWORD dwAttribute, register LPWSTR pszStr)
{
    INT cch = 0;
...
VOID SetLBFont(HWND hwnd, HWND hwndLB, HANDLE hNewFont, DWORD dwViewFlags, LPXDTALINK lpStart)
{
    ...
    else
    {
        ...
        FixTabsAndThings(hwndLB, (WORD*)GetWindowLongPtr(hwnd, GWL_TABARRAY),
           dxMaxExtent, lpHead->dwAlternateFileNameExtent, dwViewFlags);
    }
}
...
VOID RightTabbedTextOut(HDC hdc, INT x, INT y, LPWSTR pLine, WORD *pTabStops,
    INT x_offset, DWORD dwAlternateFileNameExtent)
{
    INT len, cch;
    INT count = 0;
    SIZE size;
...
LPWSTR DirGetSelection(HWND hwndDir, HWND hwndView, HWND hwndLB,
    INT  iSelType, BOOL *pfDir, PINT piLastSel)
{
    LPWSTR p = NULL, pT;
    INT i;
    INT cch;
...
HWND GetDirSelData(HWND hwnd, LARGE_INTEGER *pqSelSize, INT *piSelCount, LARGE_INTEGER* pqTotalSize, 
    INT *piTotalCount, LPFILETIME *ppftLastWrite, BOOL *pisDir, BOOL *pisNet, LPWSTR pszName)
{
    INT i;
    LPXDTA lpxdta;
    LPXDTALINK lpStart;
    ...
    *pszName = CHAR_NULL;
    ...
    // fast scroll
    if (!(hwndLB = GetDlgItem(hwnd, IDCW_LISTBOX)))
        return NULL;
...
VOID GetDirStatus(HWND hwnd, LPWSTR szMessage1, LPWSTR szMessage2)
{
    ...
    hwndLB = GetDirSelData(hwnd, &qSelSize, &iSelCount, &qSize,
                           &iCount, &lpftLastWrite, &isDir, &isNet, szName);

    SetStatusText(1, SST_RESOURCE|SST_FORMAT, (LPWSTR) MAKEINTRESOURCE(IDS_STATUSMSG), 
                  iCount, ShortSizeFormatInternal(szNumBuf, qSize));

    if (hwndLB == (HWND)GetWindowLongPtr((HWND)GetWindowLongPtr(hwnd, GWL_LISTPARMS), GWL_LASTFOCUS))
    {
        SetStatusText(0, SST_RESOURCE|SST_FORMAT, (LPWSTR) MAKEINTRESOURCE(IDS_STATUSMSG2), 
...
VOID SortDirList(HWND hwndDir, LPXDTALINK lpStart, DWORD count, LPXDTA* lplpxdta)
{
    ...
    dwSort = GetWindowLongPtr((HWND)GetWindowLongPtr(hwndDir, GWL_LISTPARMS), GWL_SORT);

...
ExtensionName(int i, LPTSTR szName)
{
    ...
    *szName = TEXT('\0');
    ...
    if ((UINT)i<(UINT)iNumExtensions && 
        GetModuleFileName(extensions[i].hModule, szFullName, COUNTOF(szFullName)) && (lpName=StrRChr (szFullName, NULL, TEXT('\\'))))
...
Static VOID EnableDisconnectButton(void)
{
    INT i;
...
VOID EnableCheckTBButtons(HWND hwndActive)
{
   ...
   switch (GetWindowLongPtr(hwndActive, GWL_VIEW) & VIEW_EVERYTHING)
   {
    ...
    fEnable = ((int)GetWindowLongPtr(hwndActive, GWL_TYPE) >= 0 && HasDirWindow(hwndActive));
...
VOID
BuildDriveLine(LPWSTR* ppszTemp, DRIVEIND driveInd, BOOL fGetFloppyLabel, DWORD dwType)
{
...
VOID RefreshToolbarDrive(DRIVEIND iDriveInd)
{
    INT iSel;
    DRIVE drive;
    ...
    /* For floppy drives, when we refresh, we should pickup the drive label. */
    if (IsRemovableDrive(drive) || IsCDRomDrive(drive))
        U_VolInfo(drive);

    /* We must tell ourselves which drive we are working on. */
...
PaintDriveLine(DRAWITEMSTRUCT FAR *lpdis)
{
    ...
    /* Check rectangle: if > 1 line and dwLines > 1 then use multiline else use abbreviated */
    ...
    {
        clrBackground = GetSysColor((lpdis->itemState & ODS_SELECTED) ? COLOR_HIGHLIGHT : COLOR_WINDOW);
        ...
        //
#if 0
        if (lpszText == NULL || lpszText == (LPTSTR)-1 || lpdis->itemID == (UINT) -1)
            return;
#endif
...
Static VOID ResetToolbar(void)
{
    ...
    for (nItem=(INT)SendMessage(hwndToolbar, TB_BUTTONCOUNT, 0, 0L)-1; nItem>=0; --nItem)
        SendMessage(hwndToolbar, TB_DELETEBUTTON, nItem, 0L);

    // Add the default list of buttons
    SendMessage(hwndToolbar, TB_ADDBUTTONS, TBAR_BUTTON_COUNT, (LPARAM)(LPTBBUTTON)tbButtons);
    ...
    if (hwndActive && InitPopupMenus(0xffff, hMenu, hwndActive))
    {
        for (i=0; i<TBAR_BUTTON_COUNT; ++i)
        {
            ...
            SendMessage(hwndToolbar, TB_ENABLEBUTTON, idCommand, !(state&(MF_DISABLED|MF_GRAYED)));
        }
...
Static BOOL GetAdjustInfo(LPTBNOTIFY lpTBNotify)
{
    ...
    if ((UINT)j < TBAR_ALL_BUTTONS)
    {
        *lpButton = tbButtons[TBAR_BUTTON_COUNT-1];
        ...
        lpButton->fsState = (sAllButtons[j].idB & HIDEIT) ? TBSTATE_HIDDEN : TBSTATE_ENABLED;
...
UnlockAndReturn:
    ...
    j -= TBAR_ALL_BUTTONS;
    if (hwndExtensions && SendMessage(hwndExtensions, TB_GETBUTTON, j, (LPARAM)lpButton))
    {
        if (lpButton->fsStyle & TBSTYLE_SEP)
        ...
        if ((UINT)iExt < (UINT)iNumExtensions)
        {
            ...
            extensions[iExt].ExtProc(hwndFrame, FMEVENT_HELPSTRING, (LPARAM)(LPFMS_HELPSTRING)&tbl);
            ...
            if (extensions[iExt].bUnicode == FALSE)
            {
                ...
                MultiByteToWideChar (CP_ACP, MB_PRECOMPOSED, szAnsi, COUNTOF(szAnsi), tbl.szHelp, COUNTOF(tbl.szHelp));
            }
...
BOOL HandleToolbarRestore(LPNMTBRESTORE lpnmtRestore)
{
...
DWORD DriveListMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, UINT* puiRetVal)
{
    UINT uID;
    FMS_HELPSTRING tbl;
    INT iExt;
...
ExtensionHelp:
                        ...
                        SendMessage(hwndStatus, SB_SETTEXT, SBT_NOBORDERS|255, (LPARAM)tbl.szHelp);
...
NormalHelp:
                MenuHelp((WORD)uMsg, wParam, lParam, GetMenu(hwndFrame), hAppInstance, hwndStatus, (LPUINT)dwMenuIDs);
            }
            else
            {
                uID = uItem;
...
#define pMeasureItem ((MEASUREITEMSTRUCT FAR *)lParam)
            ...
            //
            if (-1 == pMeasureItem->itemID || !IsRemoteDrive(rgiDrive[pMeasureItem->itemID]))
            {
            ...
            pMeasureItem->itemHeight = dyDriveItem * aDriveInfo[drive].dwLines[ALTNAME_MULTI] + DRIVELIST_BORDER;
...
DWORD ShareCountStub(DWORD iType)
{
...
VOID InitToolbarButtons(VOID)
{
    ...
    for (i=1, bLastSep=TRUE; i<TBAR_BUTTON_COUNT; ++i)
    {
        ...
        {
            if (GetMenuState(hMenu, tbButtons[i].idCommand, MF_BYCOMMAND) == (UINT)-1)
                tbButtons[i].fsState = TBSTATE_HIDDEN;
            else
                bLastSep = FALSE;
        }
...
BOOL InitToolbarExtension(INT iExt)
{
    ...
    if (!extensions[iExt].ExtProc(hwndFrame, FMEVENT_TOOLBARLOAD, (LPARAM)(LPFMS_TOOLBARLOAD)&tbl))
        return FALSE;
    ...
    {
        hwndExtensions = CreateToolbarEx(hwndFrame, WS_CHILD, IDC_EXTENSIONS, 0, 
            hAppInstance, IDB_TOOLBAR, tbButtons, 0, 0,0,0,0,sizeof(TBBUTTON));
...
AddSep:
        ...
        SendMessage(hwndExtensions, TB_INSERTBUTTON, (WORD)-1, (LPARAM)(LPTBBUTTON)&extButton);
    }
    ...
    if (tbl.idBitmap)
    {
        ...
        iStart = (INT)SendMessage(hwndToolbar, TB_ADDBITMAP, tbl.cButtons, (LPARAM) &tbAddBitmap);
    }
    else
    {
        tbAddBitmap.hInst = 0;
    ...
    for (fSepLast=TRUE, i=tbl.cButtons, iBitmap=0, lpButton=tbl.lpButtons; i>0; --i, ++lpButton)
    {
...
VOID SaveRestoreToolbar(BOOL bSave)
{
    ...
    if (bSave)
    {
        ...
        *pName = L'\0';
        ...
        SendMessage(hwndToolbar, TB_INSERTBUTTON, 0, (LPARAM)(LPTBBUTTON)tbButtons);
    }
    else
    {
        ...
        GetPrivateProfileString(szSettings, szAddons, szNULL, szNames, COUNTOF(szNames), szTheINIFile);
        ...
        if (bRestored)
        {
            ...
            {
                SendMessage(hwndToolbar, TB_DELETEBUTTON, nItem, 0L);
                SendMessage(hwndToolbar, TB_INSERTBUTTON, nItem, (LPARAM)(LPTBBUTTON)(&tbButtons[ICONNECTIONS]));
            }
